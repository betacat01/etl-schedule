...
def get_option_parser():
    ...
    parser.add_option("-n", "--instance", dest="mysql_instance", action="store", help="mysql instance")
...
def get_mysql_config(mysql_instance_name):
...
def get_username_password():
    ...
    db_info = get_mysql_config(mysql_instance_name)
...
def run_mysql_command(command):
    ...
    print mysql_path
    run_command = mysql_path + "/bin/mysql -u" + username + " -p" + password + " -h" + host + " -P" + str(port) + " -e \"" + command + "\""
...
def load_mysql(db, columns, tmpdata):
    ...
    split_command = "split -d -a 4 -l 20000 "+ tmpdata + " " + split_dir + "/" + split_prefix
    print split_command
    ...
    if split_result == 0:
        for file in os.listdir(split_dir):
            ...
            command = "load data local infile '" + split_dir + "/" + file + "' INTO TABLE " + db \
              + " CHARACTER SET 'utf8mb4' fields terminated by '" + DATA_SPLIT +"' (" + columns + ")"
